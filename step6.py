"""
Bước 6: Chatbot với LangChain + OpenAI để so sánh ảnh bbox.
"""
...
def normalize_text_for_alignment(text: str) -> str:
    """GIỮ NGUYÊN accent (è≠e, ÈME≠EME) vì đó là sai biệt thật trong tiếng Pháp."""
    if not text:
        return ""
...
def _draw_ruler_canvas_0_100(
    ...
    *,
    ...
    """
    Vẽ thước kiểu tools_demo/bbox_splitter (thước nằm NGOÀI ảnh, không che nội dung):
    - Thước X (0..100) ở dải đen phía DƯỚI (ngoài ảnh)
    - Thước Y (0..100) ở dải đen bên TRÁI (ngoài ảnh)
    """
    img = img.convert("RGB")
    cw, ch = img.size
    canvas_w = cw + pad_left
    canvas_h = ch + pad_bottom
    ...
    # Bottom ruler (X)
    draw.rectangle([pad_left, ch, canvas_w, canvas_h], fill="black")
    for i in range(0, 101):
        x = pad_left + int(round(i * (cw / 100.0)))
        if i % 10 == 0:
            draw.line([x, ch, x, canvas_h], fill="white", width=2)
            draw.text((x - 6, ch + 3), str(i), fill="white", font=font)
        elif i % 5 == 0:
            draw.line([x, ch, x, canvas_h - 15], fill="white", width=2)
        else:
            draw.line([x, ch, x, canvas_h - 25], fill="white", width=1)

    # Left ruler (Y)
    draw.rectangle([0, 0, pad_left, ch], fill="black")
    for i in range(0, 101):
        y = int(round(i * (ch / 100.0)))
        if i % 10 == 0:
            draw.line([0, y, pad_left, y], fill="white", width=2)
            draw.text((3, y - 6), str(i), fill="white", font=font)
        elif i % 5 == 0:
            draw.line([0, y, pad_left - 15, y], fill="white", width=2)
        else:
            draw.line([0, y, pad_left - 25, y], fill="white", width=1)

    meta = {
        "content_size_px": [int(cw), int(ch)],
        "pad_left_px": int(pad_left),
        "pad_bottom_px": int(pad_bottom),
        "canvas_size_px": [int(canvas_w), int(canvas_h)],
    }
    return canvas, meta


def encode_image_b_with_ruler_to_base64(
    image_path: Path,
    *,
    max_side: int = 1024,
) -> tuple[str, dict]:
    """
    Resize ảnh content rồi vẽ thước ngoài ảnh, trả base64 + meta.
    """
    with Image.open(image_path) as img:
        img = img.convert("RGB")
        ow, oh = img.size
        scale = min(1.0, max_side / max(ow, oh))
        cw = max(1, int(round(ow * scale)))
        ch = max(1, int(round(oh * scale)))
        if (cw, ch) != (ow, oh):
            img = img.resize((cw, ch), resample=Image.Resampling.LANCZOS)
        canvas, meta = _draw_ruler_canvas_0_100(img)
        buf = io.BytesIO()
...
def convert_ruler_bboxes_to_image2_bboxes_px(
    comparison_str: str,
    *,
    content_w: int,
    content_h: int,
    ...
    """
    Convert `differences.image2_bboxes_ruler_0_100` -> `differences.image2_bboxes_px`
    (bbox pixel theo ảnh content, KHÔNG tính phần thước).
    """
    if not comparison_str:
        return comparison_str
    cleaned = strip_code_fence(comparison_str)
    try:
        data = json.loads(cleaned)
    except Exception:
        return comparison_str
    ...
    ruler_boxes = differences.get("image2_bboxes_ruler_0_100")
    if not isinstance(ruler_boxes, list):
        return comparison_str

    img2_list = differences.get("image2") or []
    if not isinstance(img2_list, list):
        return comparison_str

    cw = float(content_w)
    ch = float(content_h)

    px_boxes: list[list[float] | None] = []
    max_len = max(len(img2_list), len(ruler_boxes))
    for i in range(max_len):
        item = ruler_boxes[i] if i < len(ruler_boxes) else None
        if item is None:
            px_boxes.append(None)
            continue
        ...
        if not (isinstance(item, (list, tuple)) and len(item) == 4):
            ...
            continue
        try:
            x0p, y0p, x1p, y1p = [float(v) for v in item]
        except Exception:
            px_boxes.append(None)
            continue

        x0 = max(0.0, min(cw, x0p / 100.0 * cw))
        y0 = max(0.0, min(ch, y0p / 100.0 * ch))
        x1 = max(0.0, min(cw, x1p / 100.0 * cw))
        y1 = max(0.0, min(ch, y1p / 100.0 * ch))
        px_boxes.append([x0, y0, x1, y1])

    differences["image2_bboxes_px"] = px_boxes
    differences.pop("image2_bboxes_ruler_0_100", None)
    return json.dumps(data, ensure_ascii=False, indent=2)
...
def encode_image_to_base64_with_resize(
    ...
    *,
    ...
    """
    Resize ảnh trước khi gửi cho model để coordinate của GPT ổn định hơn.

    Trả về:
      - base64 của ảnh PNG đã resize (có vẽ lưới thước cũ)
      - (w, h) của ảnh resize theo pixel
      - `image_b_ruler_origin_px`: [ox, oy] là pixel nơi giao điểm (0,0) của thước nằm
    """
    with Image.open(image_path) as img:
        img = img.convert("RGB")
        ...
        # Coordinates returned by GPT will be relative to this origin (ox, oy).
        draw = ImageDraw.Draw(img)
        # Margin to fit labels; ensure it's not outside the image.
        ox = min(120, max(30, int(new_w * 0.08)))
        oy = min(120, max(30, int(new_h * 0.08)))
        ruler_color = (255, 70, 70)  # red-ish
        grid_color = (255, 220, 220)  # very light red for grid
        tick_color = (255, 120, 120)
        # Grid/tick step in pixels (in resized image coordinates)
        step = 100
        ...
        while x <= new_w - 1:
            ...
            if dx % (step * 2) == 0:  # label every ~200px
...
def compare_two_images(
    ...
    *,
    ...
    """
    So sánh 2 ảnh bbox có cùng ID và trả về JSON CỐ ĐỊNH.
    ...
    # Image 2 gửi cho GPT có thước 0..100 (ngoài ảnh) để model trả tọa độ ổn định hơn
    image_b_b64, image_b_ruler_meta = encode_image_b_with_ruler_to_base64(image_b_path, max_side=1024)
...
- NEWly added text/watermarks/stamps in Image 2 that do not exist in Image 1 (e.g. "TRANSFORMÉE DANS...", large background text, labels)
...
- **image2** (produit fini): ONLY provide text that is DIFFERENT from brief (Image 1):
  ...
  - If Image 2 contains COMPLETELY NEW text (like a huge watermark or new label) that has no equivalent in Image 1 → add a NEW entry where `image1` is `""` and `image2` contains the full new text
...
"""
        ...
        HumanMessage(
            content=[
                ...
                },
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/png;base64,{image_a_b64}",
                        "detail": "high",
                    },
                },
                {
                ...
                },
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/png;base64,{image_b_b64}",
                        "detail": "high",
                    },
                },
            ]
    ...
    return content, image_b_ruler_meta


def step6_compare_with_gpt(
    compare_dir: Path,
    *,
    model: str = "gpt-5.2",
    temperature: float = 0.0,
    ...
    """
    So sánh các cặp bbox có ID trùng bằng GPT.
    ...
    """
    print("=" * 60)
    print("BƯỚC 6: SO SÁNH BBOX BẰNG GPT")
    print("=" * 60)
    ...
    def process_id(idx_id_tuple):
        ...
        try:
            comparison, image_b_ruler_meta = compare_two_images(
            ...
            # Convert bbox 0..100 -> bbox pixel (content) để step7 dùng anchor tìm xung quanh
            try:
                content_size = (image_b_ruler_meta or {}).get("content_size_px")
                if isinstance(content_size, list) and len(content_size) == 2:
                    comparison = convert_ruler_bboxes_to_image2_bboxes_px(
                        comparison,
                        content_w=int(content_size[0]),
                        content_h=int(content_size[1]),
                    )
            except Exception:
                pass
            ...
            return {
                ...
                "image_a": str(image_a),
                "image_b": str(image_b),
                "image_b_ruler_meta": image_b_ruler_meta,
    ...
    # Use ThreadPoolExecutor for parallel comparisons
    max_workers = min(10, len(ids_to_compare))  # Limit to 10 concurrent requests
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Bước 6: So sánh bbox bằng GPT")
    parser.add_argument(
        "--compare-dir",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt-5.2",
        help="OpenAI model (mặc định: gpt-5.1)",
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    # Chạy step 6
    results = step6_compare_with_gpt(
